...
export const TechBadge: React.FC<TechBadgeProps> = ({ tech }) => {
  ...
  if (iconPath) {
    const isExpress = iconPath.includes('express');
    return (
      <span className="flex items-center gap-2 px-3 py-1 text-xs md:text-sm font-semibold text-gray-300 bg-black/50 rounded-lg border border-white/5 transition-colors group-hover:border-purple-500/30 group-hover:text-white shrink-0">
        <img 
          src={`https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/${iconPath}`} 
          alt={tech} 
          className={`w-4 h-4 opacity-100 ${isExpress ? 'invert hue-rotate-180 brightness-200 contrast-200' : 'opacity-80'}`}
