...
const LightPillar = ({
  ...
  // BIT RAIN (SPACE BACKGROUND) EFFECT
  useEffect(() => {
    ...
    const animate = () => {
      // Create fade effect
      ctx.clearRect(0, 0, canvas.width, canvas.height); // Use clearRect so it doesn't block the lightpillar behind it if needed, or fillRect if we want the dark gradient overlay
  ...
  if (!webGLSupported) {
    return (
      <div 
        className={`absolute inset-0 w-full h-full flex items-center justify-center bg-black/10 text-[#888] text-sm ${className}`} 
        style={{ mixBlendMode }}
      >
        WebGL not supported
      </div>
